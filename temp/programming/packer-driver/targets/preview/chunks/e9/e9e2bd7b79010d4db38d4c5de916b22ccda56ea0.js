      ...
      _export("WheelController", WheelController = (_dec = ccclass('WheelController'), _dec2 = property(Node), _dec3 = property(Node), _dec4 = property(Node), _dec5 = property(ParticleSystem2D), _dec(_class = (_class2 = class WheelController extends Component {
        ...
        start() {
          if (this.wheelGlow) {
            ...
            var opacity = this.wheelGlow.getComponent(UIOpacity) || this.wheelGlow.addComponent(UIOpacity);
        ...
        spinTo(resultType, onComplete) {
          ...
          var rotation = this.calculateRotationForType(resultType); // считаем угол
        ...
        calculateRotationForType(resultType) {
          var baseSpins = (_crd && GameConfig === void 0 ? (_reportPossibleCrUseOfGameConfig({
            error: Error()
          }), GameConfig) : GameConfig).WHEEL_SPINS * 360; // обязательно делаем несколько полных оборотов

          var sectorSize = 360 / this.sectors.length; // 45 - размер одного сектора

          var halfSector = sectorSize / 2; // 22.5 - соответственно середина сектора
          ...
          var randomIndex = availableIndexes[Math.floor(Math.random() * availableIndexes.length)]; // выбираем случайный из списка подходящих, лучше было бы использовать RandomService

          var pointerAngle = 90; // угол нахождения стрелки

          var finalAngle = pointerAngle - (randomIndex * sectorSize + halfSector); // финальный угол на который нужно повернуть колесо, чтобы сектор оказался под стрелкой
