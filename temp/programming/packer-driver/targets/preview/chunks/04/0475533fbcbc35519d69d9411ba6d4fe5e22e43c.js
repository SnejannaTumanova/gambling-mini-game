  ...
  return {
    ...
    execute: function () {
      ...
      _export("GameService", GameService = class GameService {
        ...
        rollWin(chance) {
          if (chance === void 0) {
            chance = (_crd && GameConfig === void 0 ? (_reportPossibleCrUseOfGameConfig({
              error: Error()
        ...
        resolveRound(balance, bet, isWin) {
          ...
          var strategy = isWin ? this.winStrategy : this.loseStrategy;
          var nextBalance = strategy.apply(balance, bet);
          var reward = strategy.getReward(bet);
          var resultType = isWin ? bet.type : bet.type === 'red' ? 'black' : 'red';
          return {
            isWin,
            reward,
            nextBalance,
